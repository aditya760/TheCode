...
func GetAWSparams() *AWSParams{
    ...
    availablityZones := getListOfAvailabilityZones(regionName)
...
func getListOfAvailabilityZones(regionName string) map[int] string {
  sess, err := session.NewSession(&aws.Config{
      Region: aws.String(regionName)},
  )
  // Create an EC2 service client.
  ...
  fmt.Println("Success", resultAvalZones.AvailabilityZones)
   ...
   avalZoneNumber := 0
    ...
    if len(chosenZoneNumbers) < 3 {
      ...
      chosenAvalZones = getListOfAvailabilityZones(regionName)
